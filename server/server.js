...
wss1.on("connection", (ws, req) => {
  ...
  console.log(`Client ${addr} connected to /webrtc`);
  ws.on("close", () => console.log(`Client ${addr} disconnected from /webrtc`));
...
wss2.on("connection", (ws, req) => {
  const addr = `${req.socket.remoteAddress}:${req.socket.remotePort}`;
  console.log(`Client ${addr} connected to /websocket`);
  ws.on("close", () =>
    console.log(`Client ${addr} disconnected from /websocket`)
  );
  ws.on("message", (message) => {
    console.debug(`Received ${message.length} bytes from ${addr}`);
    ws.send(message);
